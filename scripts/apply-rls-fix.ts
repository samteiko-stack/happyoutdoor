/**
 * Apply 002_fix_rls_recursion.sql using the Supabase database password.
...
if (!url || !dbPassword) {
  console.error(
    "Add SUPABASE_DB_PASSWORD to .env.local (Supabase → Project Settings → Database).\n" +
      "Or paste supabase/migrations/002_fix_rls_recursion.sql into the Supabase SQL Editor."
  );
  process.exit(1);
}

const projectRef = new URL(url).hostname.split(".")[0];
const sqlPath = path.join(process.cwd(), "supabase/migrations/002_fix_rls_recursion.sql");
...
async function main() {
  ...
  console.error("Failed to apply RLS fix:", lastError instanceof Error ? lastError.message : lastError);
  process.exit(1);
}
